...
const GridScan: React.FC<GridScanProps> = ({
  ...
  const [_uiFaceActive] = useState(false);
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (
        ...
        (DeviceOrientationEvent as unknown as Record<string, unknown>).requestPermission
      ) {
        try {
          await ((DeviceOrientationEvent as unknown as Record<string, (() => Promise<void>)>).requestPermission)();
        } catch { /* ignore */ }
      }
    };
    const onEnter = () => {
  ...
  }, [_uiFaceActive, snapBackDelay, scanOnClick, enableGyro]);
